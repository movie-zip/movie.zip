<template>
  <div>
    <h1 class="nickname">{{ nickname }}님이 볼 영화</h1>
    <hr>
    <div v-if="movies && movies.length">
      <Carousel :items-to-show="3" :wrap-around="true">
        <Slide v-for="movie in movies" :key="movie.id">
          <ToWatchMovieListItem :movie="movie" />
        </Slide>
        <template #addons>
          <Navigation />
        </template>
...
import ToWatchMovieListItem from '@/components/movies/ToWatchMovieListItem.vue'
...
const getUserProfile = async () => {  // 현재 들어간 프로필 사용자의 정보를 가져온다
  try {
    ...
    categoryId.value = response.data.my_categories[2].id  // 현재 들어간 카테고리 아이디
  } catch (error) {
    console.log(error)
  }
}
